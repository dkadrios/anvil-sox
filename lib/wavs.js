    ...
    gulp.task('build',
        ...
        function () {
          ...
          function processPanning(file) {
            ...
            if (panningValue > 0) {
              // Calculate "left"
              result[0] = Math.abs((panningValue + 10) / 10 - 2);

            } else if (panningValue < 0) {
              // Calculate "right"
              result[1] = 1 + panningValue / 10;
            }
          ...
          return gulp.src(folderTmp + pattern, {read: false})
              ...
              .pipe(tap(function (file) {
                var result = processPanning(file);

                file.panLeft = result[0];
                file.panRight = result[1];
              }))
